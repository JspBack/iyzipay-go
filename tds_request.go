...
func (i iyzipayClient) InitilizeTDSPayment(req *InitTDSRequest) (response InitTDSResponse, decodedHtmlContent string, err error) {
	...
	httpresp, err := utils.DoRequest(requestData, i.client, i.baseURI, i.apiKey, i.apiSecret, utils.TDSInitilizeURI)
	if err != nil {
		return response, decodedHtmlContent, err
	}
	...
	if err != nil {
		return response, decodedHtmlContent, errors.New("failed to unmarshal response")
	}
	...
	if response.Status == "failure" {
		...
		return response, decodedHtmlContent, errors.New(
			"{" +
...
func (i iyzipayClient) FinalizeTDSPayment(req *TDSPaymentRequest) (response TDSPaymentResponse, err error) {
	...
	httpresp, err := utils.DoRequest(requestData, i.client, i.baseURI, i.apiKey, i.apiSecret, utils.TDSFinalizeURI)
	if err != nil {
		return response, err
	}
